...
const savedDesignEntity = {
  async list(sort = '-updated_date', limit) {
    ...
    // Metadata only — never pull the (potentially large) `layers` JSON here.
    // Listing designs shouldn't download every design's embedded images; the
    // full row is fetched via get() only when a design is actually opened.
    let query = client
      .from('saved_designs')
